...
z3::expr listsetsolver::compute_tr_closure(predicate &pred) {
        ...
        delta = delta.substitute(src_pars, dst_pars);
        std::cout << "phi_r: " << delta << std::endl;

        // 1. get strt(delta)
        z3::expr strt_phi_r = get_strt(delta);
        std::cout << "strt_phi_r: " << strt_phi_r << std::endl;
        // 2. case by case tr
 ...
 * @return strt(phi_r)
 */
z3::expr listsetsolver::get_strt(z3::expr phi_r) {
        std::cout << "get strt \n";
        ...
        z3::expr term_s = phi_r1.arg(1).arg(1).arg(0);
        ...
        if (is_sat) {
                ...
                return phi_r1 && strt_phi_r2;
        }

        // display(matrix, set_vars, "str_phi_r.dot");

        return phi_r1 && z3_ctx().bool_val(false);
}
...
z3::expr listsetsolver::get_card(int i, z3::expr_vector &set_vars) {
        z3::sort set_int_s = z3_ctx().uninterpreted_sort("SetInt");
        z3::sort int_s = z3_ctx().int_sort();
        std::string name = "min";
        if (i&2) name = "max";
        z3::func_decl f = z3_ctx().function(name.c_str(), set_int_s, int_s);
        return f(set_vars[i&1]);
}
