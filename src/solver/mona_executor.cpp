 ...
 * @return true, if sat
 */
bool mona_executor::execute(std::map<std::string, std::string>& model) {
        std::string cmd = "mona ";
        cmd.append(m_args);
...
bool mona_executor::parse_model(std::string &str,  std::map<std::string, std::string>& model) {
...
void mona_executor::get_minus_val(std::string &value) {
        if (value.find('{') == -1) {
