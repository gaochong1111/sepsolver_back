...
FA FA::get_flow(int accept) {
        ...
        // add vertex
        result.add_states(visited.size(), "q_");
        ...
        result.set_accept_states(accept_states);
 ...
 */
void FA::get_valid_states(int accept_state, std::set<int>& valid_ids) {
...
z3::expr FA::to_expr(z3::context& ctx, int accept_state, std::set<int>& x_ids, std::set<z3::expr, exprcomp>& tpaq_set) {
        ...
        get_valid_states(accept_state, valid_ids);
        ...
        while(work_list.size() > 0) {
                ...
                for (; i_start != i_end; ++i_start) {
                        ...
                        // make var t_p_a_q
                        z3::expr t_p_a_q = expr_tool::mk_int_var(ctx, "t_", i_src, i_edge, i);
                ...
                for (; o_start != o_end; ++o_start) {
                        ...
                        if (valid_ids.find(i_target) != valid_ids.end()) {
                                ...
                                int i_edge = m_fa[boost::graph_bundle][key];
                                // make var t_p_a_q
                                z3::expr t_p_a_q = expr_tool::mk_int_var(ctx, "t_", i, i_edge, i_target);
        ...
        while(it != id_to_neighbors.end()) {
                int i = it->first;
                z3::expr u_i = expr_tool::mk_int_var(ctx, "u_", i);
                if (i != 0) {
                        ...
                        while(it1 != neighbors.end()) {
                                int j = it1->first;
                                z3::expr u_j = expr_tool::mk_int_var(ctx, "u_", j);
...
void FA::print(std::string name) {
        ...
        std::set<int> valid_ids;
        for (int i=0; i<m_accept_states.size(); i++) {
                get_valid_states(m_accept_states[i], valid_ids);
        }
 ...
 *
 */
bool FA::has_path(int src, int dst,  std::set<int>& ids, std::map<std::string, int>& edge_count) {
        ...
        while(work_list.size() > 0) {
                ...
                for (; i_start != i_end; ++i_start) {
                        ...
                        if (ids.find(target) != ids.end()) {
                                ...
                                std::string edge_key = vec_to_str(edge_info);
        ...
        while(work_list.size() != 0) {
                ...
                for (; i_start != i_end; ++i_start) {
                        ...
                        if (ids.find(i_target) != ids.end()) {
                                ...
                                z3::expr t_paq = expr_tool::mk_int_var(ctx, "t_", cur_state+1, id, i_target+1);
                                ...
                                if (state_set.find(i_target) == state_set.end()) {
                                        if (i_edge_count > 0) {
                                                if (!find_path && has_path(i_target, cur_state, ids, edge_count)) {
                                                        i_out = i_start;
                                                        i_dst = i_target;
                                                        key = t_paq_name;
                                                        find_path = true;
